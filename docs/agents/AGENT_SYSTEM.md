...
- 共享状态统一回写到仓库内文档
- 代码交付统一通过 PR 收口
- 涉及 UI 页面开发或页面结构设计的任务，可在总控正式派单后调用 `Figma MCP` 新建设计文件、页面和 `frame`
- `Figma MCP` 仅用于 UI 设计资产与页面结构管理，不替代仓库内共享文档作为单一事实源
- `Figma` 中形成的页面结构、状态稿和设计资产，必须回收到共享文档或 PR 描述后才视为正式生效
