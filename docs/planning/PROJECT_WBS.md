...
- iPhone 端首次引导、参数配置、基础历史页、帮助与支持入口
...
| 任务 | 优先级 | 状态 | 交付物 | 验收标准 |
| --- | --- | --- | --- | --- |
| 实现 `TimerManager` | P0 | 未开始 | 计时管理代码 | 支持开始倒计时、休息倒计时、节奏提示，且同一时刻只保留一个激活中的训练控制计时上下文 |
| 接入 3 秒训练前倒计时 | P0 | 未开始 | 倒计时逻辑 | 倒计时结束自动进入训练；倒计时态仅允许取消，不开放暂停、结束训练和 `+1 / -1` |
| 接入组间休息倒计时 | P0 | 未开始 | 休息逻辑 | 休息结束自动进入下一组；休息态暂停恢复后继续剩余时间而不重置 |
| 实现“提前开始下一组” | P0 | 未开始 | 快捷控制逻辑 | 可跳过剩余休息时间，且与休息自然结束共用同一条进入下一组语义，不重复触发反馈 |
| 实现 `HapticManager` | P0 | 未开始 | 震动封装 | 支持倒计时、计数、单组完成、进入下一组、训练完成等不同事件反馈 |
| 实现节奏提示与计数震动冲突处理 | P1 | 未开始 | 反馈优先级逻辑 | 遵循 `训练完成` > `单组完成 / 进入下一组` > `单次计数成功` > `节奏提示`，高优先级覆盖低优先级 |

#### 当前待办

- 已完成 `TASK_003` 边界冻结，确认 `TimerManager` 统一覆盖 `countdown`、`resting` 与训练态节奏提示，但同一时刻只保留一个激活中的训练控制计时上下文
- 已完成 `TASK_003` 边界冻结，确认 `HapticManager` 只负责事件反馈与冲突优先级，不直接驱动 `WorkoutSessionViewModel` 状态流转
- 已完成产品口径补齐，确认倒计时不可暂停、休息提前开始与自然结束共用同一状态语义，结束训练确认文案已冻结
- 下一步先定义计时与震动事件枚举，再进入实现阶段
...
| 任务 | 优先级 | 状态 | 交付物 | 验收标准 |
| --- | --- | --- | --- | --- |
| 定义 `SquatDetectionManager` 接口 | P0 | 未开始 | 协议与事件流 | 只输出事件，不直接改 UI |
| 实现模拟识别模式 | P0 | 未开始 | 调试模式 | 无真实传感器时可联调整体流程 |
| 实现简化识别状态机 | P1 | 未开始 | MVP 识别逻辑 | 正常深蹲能计数 |
| 增加 cooldown 机制 | P1 | 未开始 | 去重逻辑 | 单动作不会重复计数 |
| 增加内部阈值配置 | P1 | 未开始 | 可调参数结构 | 可进行真机调参 |
...
- 状态：`未开始`
- 阶段目标：让 iPhone 端承担配置、引导和辅助能力

| 任务 | 优先级 | 状态 | 交付物 | 验收标准 |
| --- | --- | --- | --- | --- |
| 实现首次启动引导页 | P0 | 未开始 | iPhone onboarding | 用户可理解手表端训练路径与权限原因 |
| 实现参数配置页 | P0 | 未开始 | 配置 UI | 默认参数可同步到 Watch |
| 实现基础历史页 | P1 | 未开始 | 训练记录列表 | 可看到最近训练摘要 |
| 实现帮助与支持页 | P1 | 未开始 | 帮助中心 | 可访问 FAQ、支持链接、隐私政策 |
| 实现基础设置页 | P1 | 未开始 | 设置 UI | 可管理默认参数和基础开关 |
...
| 为识别状态机编写测试 | P1 | 未开始 | 测试代码 | cooldown、闭环状态、阈值逻辑可验证 |
...
3. 建立共享模型和同步模型
4. 跑通 Watch 端训练主流程
5. 实现 iPhone 端最小 companion：引导页 + 参数配置
6. 接入权限、隐私和基础提审准备
...
| companion 同步边界未定 | 中风险 | 影响 iPhone/Watch 职责划分 | 先定义 Watch 为训练真源 |
