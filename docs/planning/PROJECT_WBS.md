...
- 进入 `6.4` 前，先完成首个 PR 的范围冻结与未纳管项目基线收口判断
- PR 范围收口完成后，再进入 `TimerManager`、训练前倒计时和组间休息真实计时
...
| 任务 | 优先级 | 状态 | 交付物 | 验收标准 |
| --- | --- | --- | --- | --- |
| 实现 `TimerManager` | P0 | 未开始 | 计时管理代码 | 支持开始倒计时、休息倒计时、节奏提示 |
| 接入 3 秒训练前倒计时 | P0 | 未开始 | 倒计时逻辑 | 倒计时结束自动进入训练 |
| 接入组间休息倒计时 | P0 | 未开始 | 休息逻辑 | 休息结束自动进入下一组 |
| 实现“提前开始下一组” | P0 | 未开始 | 快捷控制逻辑 | 可跳过剩余休息时间 |
| 实现 `HapticManager` | P0 | 未开始 | 震动封装 | 支持不同事件反馈 |
| 实现节奏提示与计数震动冲突处理 | P1 | 未开始 | 反馈优先级逻辑 | 单次计数反馈优先于节奏提示 |
