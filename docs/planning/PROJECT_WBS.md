...
| 收敛 `TASK_016` 真机失败需求与修复边界 | P0 | 已完成 | `TASK_017` | 已区分立即修复 bug、验收口径修订与后续新增范围 |
| 补齐动作识别可观测性（内部诊断） | P0 | 已完成 | `TASK_018` 诊断补丁与回传模板 | Debug/Beta 可见采样、校准、live 指标和未计数原因分类；正式路径不扩范围 |
| 修复 `TASK_016` 真机失败阻塞项 | P0 | 未开始 | 修复提交 | 正式路径无模拟识别 / `+1/-1`，校准体验可理解，正常深蹲可驱动 `live` 计数 |
| 修复校准计次状态机稳健性（`TASK_019`） | P0 | 进行中 | 修复提交与真机复测回传 | 校准可稳定完成计次并生成 profile，随后进入 `live` 计数链路 |
...
- 用户已在 `2026-04-21` 完成真实设备回传，结论为不通过：
  - 首次校准缺少合理引导、完成反馈不明确，且采样窗口过短
  - 正常深蹲未能驱动 `live` 计数
  - 正式路径仍暴露“模拟识别”与 `+1 / -1`
  - 半蹲 / 抬腕 / 晃动防误触因基础识别失败而无法有效判定
- 已新增 `TASK_017` 收敛真机失败后的需求口径：
  - 当前必须修复正式路径 UI、校准体验 / 采样窗口和真实识别链路
  - 校准复用仍按“首次无校准结果时校准、后续复用”执行
  - iPhone 端重置校准入口拆为后续新增任务，不混入当前 Watch 修复
- 已在 `2026-04-24` 完成 `TASK_018` 真机诊断回传：
  - `CoreMotion` 可用且已收到采样（`CMDeviceMotion` 有数据）
  - 校准阶段观测到 `depthAngle/pitchDelta` 变化，但已识别校准次数仍为 `0/10`
  - `fullDepthAngle/fullDepthPitchDelta` 未生成，`live` 阶段诊断字段为空
  - 结论：本轮不通过，问题主阻塞位于“校准计次未完成”，而非采样链路中断
- `TASK_019` 已启动并落地首轮代码修复：校准“到底后回正计次”已改为“动态回正阈值 + 短稳定窗口”，避免瞬时双阈值过严导致长期停留 `0/10`
- `2026-04-25` 第二轮 `TASK_019` 修复已落地：校准回正判定支持“单通道达标 + 另一通道接近站立”的容错；live 识别新增动态回正阈值策略并下调默认稳定/冷却门槛；内部诊断稳定输出 `未计数原因: <rawKey>（中文）`；补齐“非训练态不计数”最小测试承接
- `2026-04-28` 第二轮真机复测回传：校准与 profile 链路可用，但仍出现 `resting` 抬手误计、标准深蹲漏计，以及失败动作未稳定输出 `noRepReason` 原文的问题
- `2026-04-28` 第三轮 `TASK_019` 小范围修复已落地：`standing` 抬腕帧禁止触发下降、`live` 回正判定阈值进一步收敛、内部诊断同屏展示最近两条 raw 原文、补齐 `paused` 不计数与“抬腕/漏计场景两条 raw 原文”测试承接
- 当前下一步：执行下一轮真机诊断回传，重点确认“抬手误计是否显著下降、标准深蹲漏计是否继续下降、失败场景是否稳定带出两条 raw 原文”
