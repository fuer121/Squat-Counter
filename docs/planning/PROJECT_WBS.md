...
- 已完成 `TASK_008` 实现契约补齐，正式明确允许修改的 iPhone 代码与测试文件范围、`WorkoutConfig` 本地持久化与页面层边界、最小验证方式，以及 `6.7` / 历史页 / 实时训练控制 / 实时状态镜像不纳入本任务
- `6.6` 可继续推进实现，但前提是总控 checkpoint 接受 `TASK_008` 的实现文件范围，并接受“帮助 / 隐私最终 URL 未冻结不阻塞页面骨架实现、但阻塞提审收口”的前置判断
