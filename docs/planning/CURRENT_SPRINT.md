...
- 最新真机回传：`2026-04-25` 复测显示 `CoreMotion` 与 calibration profile 已打通，但校准计次仍可能卡在 `1/10`，且 `live` 计数存在明显漏计（`3` 次标准动作仅 `+1`）；同时失败动作未稳定回传 `noRepReason` 原文 key
- `TASK_019` 进展：已完成第二轮状态机修复与诊断补强（校准回正判定放宽、live 回正阈值策略、`noRepReason` 原文稳定输出、非训练态不计数测试承接），等待下一轮真实设备复测确认
...
- 当前下一步：基于 `TASK_019` 第二轮补丁执行下一轮真实设备诊断回传，优先确认“校准连续计次是否稳定推进到目标次数”“标准深蹲是否明显减轻漏计”“失败动作是否稳定输出 `未计数原因: <rawKey>`”；iPhone 端重置校准入口后续单独立项
